...
(defn confirm-users-email! [user-store user confirmation-id]
  (log/debug (format "confirmation-ids match. Confirming user's email."))
  (user/confirm-email! user-store user)
  (conf/revoke! @storage/confirmation-store confirmation-id)
...
(defn confirmation-id-exists? [confirmation-id]
  (not (= nil (conf/fetch @storage/confirmation-store confirmation-id))))

(defn confirm-email-with-id [user-store request]
  (if (confirmation-id-exists? (get-in request [:params :confirmation-id]))
    (if (u/signed-in? request)
      (let [user-email (get-in request [:session :user-login])
        ...
        (if (= (:login confirmation) (:login user))
          (confirm-users-email! user-store user (:confirmation-id confirmation)) 
