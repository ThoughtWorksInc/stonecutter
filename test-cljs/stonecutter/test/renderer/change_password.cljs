...
(deftest render!
         ...
         (testing "current password rendering"
                  (testing "error blank, gives it an invalid class and error message"
                           ...
                           (tu/has-message-on-selector cp-test/form-row-current-password-error-class (get-in r/error-to-message [:current-password :blank])))
                  ...
                  (testing "error too-short, gives it an invalid class and error message"
                           (let [state (assoc-in default-state [:current-password :error] :too-short)]
                             (r/render! state))
                           (tu/test-field-has-class cp-test/current-password-field cp-test/field-invalid-class)
                           (tu/has-message-on-selector cp-test/form-row-current-password-error-class (get-in r/error-to-message [:current-password :too-short])))
         ...
         (testing "new password rendering"
                  (testing "error blank, gives it an invalid class and error message"
                           ...
                           (tu/has-message-on-selector cp-test/form-row-new-password-error-class (get-in r/error-to-message [:new-password :blank])))

                  (testing "error too-short, gives it an invalid class and error message"
                           (let [state (assoc-in default-state [:new-password :error] :too-short)]
                             (r/render! state))
                           (tu/test-field-has-class cp-test/new-password-field cp-test/field-invalid-class)
                           (tu/has-message-on-selector cp-test/form-row-new-password-error-class (get-in r/error-to-message [:new-password :too-short])))
                  ...
                  (testing "error unchaged, gives it an invalid class and error message"
                           (let [state (assoc-in default-state [:new-password :error] :unchanged)]
                             (r/render! state))
                           (tu/test-field-has-class cp-test/new-password-field cp-test/field-invalid-class)
                           (tu/has-message-on-selector cp-test/form-row-new-password-error-class (get-in r/error-to-message [:new-password :unchanged])))
